<template>
  <div class="variate">
    ...
    <div class="content-main whiteBox" ref="conBox">
      <div class="float-div">
        <!-- 文字 -->
        <div class="content-title">
          ...
          <template v-if="!deleteMul">
            <el-button class="del" size="mini" @click="deleteMul=true" v-show="tableMode">批量选择</el-button>
          </template>
          <template v-else>
            <el-button
          ...
          >
            ...
            <el-table-column prop="value" label="当前值">
              <template slot-scope="scope">
                <span>{{scope.row.value>=0 ? selectType==0? Math.round(scope.row.value * 1000) / 1000:Math.floor(scope.row.value) : ' '}}&nbsp;&nbsp;&nbsp;({{scope.row.unit}})</span>
              </template>
            </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label>
          <el-radio-group v-model="addVariateData.rw">
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this.conHeight = this.$refs.conBox ? this.$refs.conBox.offsetHeight - 55 - 65 : 0
    });
  ...
  methods: {
    ...
    async getLeadDetail() {
      try {
        ...
        if (res.data.code === 0) {
          this.variateData = []
          this.leadDetailList = res.data.data.fileRecordsVOList
