    ...
    >
      ...
      <el-table-column prop="level" label="告警级别">
        <template slot-scope="scope">
          <span class="one-type warn-btn" v-if="scope.row.level == 1"
            >一级</span
          >
          <span class="two-type warn-btn" v-else-if="scope.row.level == 2"
            >二级</span
          >
          <span class="three-type warn-btn" v-else-if="scope.row.level == 3"
            >三级</span
          >
          <span class="four-type warn-btn" v-else-if="scope.row.level == 4"
            >四级</span
          >
          <span class="five-type warn-btn" v-else-if="scope.row.level == 5"
            >五级</span
          >
          <span class="six-type warn-btn" v-else-if="scope.row.level == 6"
            >六级</span
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="触发条件" prop="triggerCondition">
          <el-select
            v-model="warnForm.triggerCondition"
            placeholder="请选择触发条件"
          >
            <el-option
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          class="cancel"
          @click="editWarnVisible = false"
          style="margin-left: 200px"
          >取 消</el-button
        >
        <el-button class="confirm" :loading="ifSubmit" @click="editWarnSubmit"
          >提交</el-button
        >
      </span>
    </el-dialog>
