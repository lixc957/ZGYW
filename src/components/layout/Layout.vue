    ...
    <el-aside class="bx-size aside" style="width: auto; max-width: 201px">
      <div
        style="
          width: 100%;
          height: 60px;
          background-color: rgba(42, 68, 95);
          position: relative;
        "
      >
        <div
      ...
      >
        <el-menu
          ...
          v-if="menuShow"
          class="el-menu-vertical-demo menuBox"
          unique-opened
          :text-color="'#fff'"
        ...
        >
          ...
          <el-menu-item index="/layout/workRule">
            <span slot="title">工单运维</span>
          </el-menu-item>
          <el-menu-item index="/layout/order">
            <span slot="title">工单运维</span>
          </el-menu-item>
          <el-menu-item index="/layout/orderPlatform">
            <span slot="title">工单平台</span>
          </el-menu-item>
        </el-menu>
      </div>
    </el-aside>
        ...
        <div class="icon-box" @click="changeStatus">
          ...
          <i
            :class="{
              icon: true,
              iconfont: true,
              'icon-shousuocaidan-copy': isCollapse,
              'icon-shousuocaidan': !isCollapse
            }"
          ></i>
        </div>
        <div style="position: absolute; right: 10px; top: 16px">
          ...
          >
            ...
            <el-dropdown-menu slot="dropdown" style="margin-top: 0">
              <el-dropdown-item @click.native="checkInfo"
...
export default {
  ...
  computed: {
    ...
    watchSiteChange() {
      ...
      if (this.$store.state.site.siteScreenUrl) {
        this.getNav()
      }
  ...
  methods: {
    ...
    selectMenu(index, indexPath) {
      try {
        ...
        this.$router.push(index.indexOf('Screen') > -1 ? url : index)
        if (index.indexOf('Screen') > -1) {
          this.menuSelect = url
          ...
          if (res.data.code != 0) {
            ...
            if (this.$store.state.site.siteScreenUrl) {
              this.menus.forEach((item) => {
                if (item.menuId == 29) {
                  item.childrenMenu.forEach((el) => {
                    if (el.menuId == 44) {
                      el.url = this.$store.state.site.siteScreenUrl
                      return
                    }
                    return
                  })
...
.aside /deep/ .el-menu-vertical-demo .el-menu-item:focus {
  ...
  // background-color: @active-radio !important;
  // color: @font-color!important;
  color: @active-radio!important;
}
...
.aside /deep/ .el-menu-vertical-demo li .el-submenu__title:hover {
  outline: 0;
