<template>
  ...
  <div style="position: relative; height: 100%" id="warn-config">
    ...
    <div class="sec">
      <div class="sec-left whiteBox">
        <div class="tree tree-box">
          <!-- <el-tree
            :data="equipTree"
            ref="groupTree"
            node-key="id"
      ...
      <div class="sec-right whiteBox">
        <div class="warn-top">
          <span style="font-size: 28px; color: rgb(62, 151, 205)">{{
            equipmentName
          }}</span>
          <el-switch
            style="margin: 0 10px"
          ...
          ></el-switch>
          <span style="font-size: 14px">告警开关</span>
        </div>
        <div class="warn-detail" style="padding: 0 10px">
          <el-select
            v-model="type"
            filterable
            clearable
        ...
        <div class="warn-table whiteBox" ref="conBox">
          <div class="shadow">
            <el-table
    ...
    >
      ...
      >
        ...
        <el-form-item label="变量编码" v-else>{{
          addConfigForm.simplifiedCode
        }}</el-form-item>
        ...
        <el-form-item
          ...
          class="inline-input"
          v-if="
            addConfigForm.trigger == '数值大于A' ||
            addConfigForm.trigger == '数值小于A'
          "
        >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="cancel" @click="addConfigVisible = false"
          >取 消</el-button
        >
    ...
    >
      ...
      <span>是否确定删除此告警？</span>
      <span slot="footer" class="dialog-footer">
        <el-button class="cancel" @click="delWarnVisible = false"
          >取 消</el-button
        >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="cancel" @click="delMulConfigVisible = false"
          >取 消</el-button
        >
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this.conHeight = this.$refs.conBox && this.$refs.conBox.offsetHeight - 58;
    });
    window.addEventListener("resize", () => {
      this.$nextTick(() => {
        this.conHeight =
          this.$refs.conBox && this.$refs.conBox.offsetHeight - 58;
      });
    });
  },
  data() {
      ...
      this.$refs.addConfigRef.validate((valid) => {
        if (valid) {
          ...
          if (!this.addConfigForm.id) {
            ...
            // 新增
            this.$http
              .post("equipment/variableMonitor/add", obj)
              .then((res) => {
                if (res.data.code == 0) {
                  this.$notify({
                    message: "新建成功",
