  ...
  <div class="reading">
    ...
    <div class="wrap">
      <div class="meter-reading" ref="reading">
        <!-- <div> -->
        <div class="meterReading-left whiteBox">
          <div class="filter">
        ...
        <div class="meterReading-right whiteBox">
...
export default {
  ...
  methods: {
    ...
    // 获取时间节点后面的的数据
    getReport(checkedData, checkedNotRender) {
      if (checkedNotRender.length === 0) {
        this.temData = checkedData;
        return;
      }
      this.loading = true;
      var this_ = this;
      ...
      if (this.type === "department") {
            ...
            .then((res) => {
              if (res.data.code === 0) {
                if (res.data.data.length == 0) {
                  this_.temData = checkedData;
                  return;
                }
                res.data.data.name=res.data.data.department;
                this_.temData = checkedData.concat(res.data.data);
                if (i < checkedNotRender.length - 1) {
                  i++;
                  fnc();
                }
              }
        ...
        this.getHistortReport(checkedData, checkedNotRender);
      } else {
        this.$http
          .get("equipment/energyBranch/checkEnergyByBatch", {
            params: {
              branchs: checkedNotRender.join(","),
              time: timeObj.time,
              tb: timeObj.tb,
              siteId: this.$store.state.site.siteId,
          ...
          .then((res) => {
            if (res.data.code === 0) {
              this.temData = checkedData.concat(res.data.data);
              for (var i = 0; i < this.temData.length; i++) {
                this.$set(
                  this.temData[i],
                  "name",
                  this.temData[i].branch || name
                );
              }
              this.getHistortReport(checkedData, checkedNotRender);
            }
    ...
    // 获取时间节点前面的数据
    getHistortReport(checkedData, checkedNotRender) {
      if (checkedNotRender.length === 0) {
        this.temData = checkedData;
        return;
      }
            ...
            .then((res) => {
              if (res.data.code === 0) {
                if (res.data.data.length == 0) {
                  ...
                  this_.temData.forEach((item, index) => {
                    if (item.name == checkedNotRender[i]) {
                      this_.$set(
                        this_.temData[index],
                        "oldValue",
                        res.data.data.value
            ...
            if (res.data.code === 0) {
              ...
              this_.temData.forEach((item, index) => {
                res.data.data.forEach((el) => {
                  if (item.name == el.branch) {
                    this_.$set(this_.temData[index], "oldValue", el.value);
                  }
                });
              });
              this.loading = false;
    ...
    selectChange(val) {
      ...
      if (this.type === "department") {
        for (var i = 0; i < checkedData.length; i++) {
          var obj = {
            name: checkedData[i].name,
          };
          this.temData.push(obj);
        }
      }
      console.log(checkedData,checkedNotRender)
      this.getReport(checkedData, checkedNotRender);
    },
    // 选中时间改变
    timeChange() {
      this.tableData = [];
