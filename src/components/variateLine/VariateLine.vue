<template>
  <div class="variate-line">
    <MyBreadcrumb
      ...
      v-if="!add_line"
    />
    <div class="select-area whiteBox" v-if="!add_line">
      <!-- 单选框 -->
      <el-radio-group
        v-model="timeSelect"
        size="middle"
      ...
      <div class="tag">
        <div class="sin-tag" v-for="(item, index) in muiOptions" :key="index">
          ...
          <i
            ...
            @click="delCompareSin(index)"
          ></i>
        </div>
      </div>
      <el-button class="confirm" @click="compareLine" v-show="muiOptions.length"
        >比对</el-button
      >
      <!-- 无站点情况下不能新增曲线 -->
      <el-button
      ...
      this.$http
        ...
        .then((res) => {
          if (res.data.code == 0) {
            ...
            this.line_total = getLineData(time, this.timeSelect, temp);
