...
export function formatMonitorData(time, type, line_Data) {
  ...
  var colorArr = ['#DB9C3F', '#9D66CC', '#ADA0FF', '#12E0E8', '#FFA0DB', '#91E0FF', '#FFF991', '#FF9500', '#9B8043', '#86B1D0']; //默认颜色数组
  var color = []
  var xAxisArrs = [];//5分钟或者一个小时的标志集合
  // 加默认颜色
  for (let i = 0; i < line_Data.length; i++) {
    code.push(line_Data[i].name)
    color.push(line_Data[i].color)
    xAxisArrs.push(line_Data[i].xaxisIndex)
  }
  obj.code = code
  obj.xAxisArrs = xAxisArrs;
  obj.data = line_Data
  ...
  for (let j = 0; j < obj1.data.length; j++) {
    ...
    // 判断颜色数组值是否存在，不存在先从颜色数组中取色，超出范围再随机颜色
    if (!color[j]) {
      if (index > 9) {
        obj1.color.push(color16());
      } else {
        obj1.color.push(colorArr[index])
        index++;
      }
    } else {
      obj1.color.push(color[j])
    }
...
// 截取指定长度字符 多出部分显示省略号
export function getString(str, len) {
  var strlen = 0;
  var s = "";
...
// 根据id查询item
export function findItemById(id, arr) {
  var item = {};
  arr.forEach(element => {
    if (element.id == id) {
      item = element;
